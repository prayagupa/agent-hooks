...
| TM-11 | Streaming egress before interception: partial model output reaches the caller before `output` (or `post_model_call`) is evaluated | E, I | In (partially open) | §12.1 covers model→host streaming (assemble before `post_model_call`, else fail closed `host_error:streaming_unsupported`). Host→caller egress before `output`: undefined | §12.1 negative path: **GAP** (no vector). Host→caller egress: **GAP** — open decision (arch-review 2026-07-09 NOW-08 / open question Q1) |
| TM-12 | Resource exhaustion: unbounded `target`/`messages` canonicalized, hashed, and deep-copied per interceptor per emission — multiplied by parallel profiles' per-interceptor snapshots | D | In | §12.3 RECOMMENDED bounds (5 MiB / depth 128); the identity path enforces the depth default fail-closed (iterative guard — a deep context denies instead of overflowing the canonicalizer's stack) | Depth: `sdk/rust/core/src/canonical.rs` depth tests. Serialized-size cap: **GAP** — host-side only, no emitter enforces a default; normative limits are an open question |
...
| §12.1 streaming fail-closed path untested; host→caller egress before `output` undefined | TM-11 | NOW-08 (decision pending) |
| No serialized-size bound enforced by any emitter (depth is enforced on the identity path) | TM-12 | open question Q7 (fixed vs tunable) |
